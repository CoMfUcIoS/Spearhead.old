...
const $config = (function() {
  ...
  return {
    ...
    listen : function(sckt) {
      _server.events.on('getConfig', ({ uToken, timeout }) => {
        _util.log('getConfig emitted uToken: ' + uToken);
        this.get({ uToken, timeout }).done((data) => {
          _server.events.trigger(sckt.id, 'config', data);
        })
        .fail((err) => {
          _util.log('$config : get : Failed: ' + uToken + ' err: ' + err);
          _server.events.trigger(sckt.id, 'config', '{}');
        });
      });

      _server.events.on('updateConfig', ({ uToken, config, timeout }) => {
        _util.log('updateConfig emitted uToken: ' + uToken);
        this.update({ uToken, config, timeout }).done((saved) => {
          _server.events.trigger(sckt.id, 'configUpdated', saved);
        });
      });
    }
